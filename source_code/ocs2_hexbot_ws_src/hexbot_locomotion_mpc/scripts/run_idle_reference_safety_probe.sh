...
CONTAINER_RUN_SCRIPT="${WORKSPACE_ROOT}/container_run_hexbot_jazzy.sh"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --translation-lookahead)
      TRANSLATION_LOOKAHEAD="$2"
      shift 2
      ;;
    --rotation-lookahead)
      ROTATION_LOOKAHEAD="$2"
      shift 2
      ;;
    --moving-goal-reference-file)
      MOVING_GOAL_REFERENCE_FILE="$2"
      shift 2
      ;;
    --auto-load-moving-goal-reference)
      AUTO_LOAD_MOVING_GOAL_REFERENCE="$2"
      shift 2
      ;;
    --use-observed-joint-target-while-moving)
      USE_OBSERVED_JOINT_TARGET_WHILE_MOVING="$2"
      shift 2
      ;;
    *)
...
if [[ "${SOLVER}" != "ipm" && "${SOLVER}" != "sqp" ]]; then
  echo "[error] unsupported solver: ${SOLVER} (expected ipm or sqp)" >&2
  exit 2
fi

LAUNCH_EXTRA_ARGS=()
if [[ -n "${TRANSLATION_LOOKAHEAD}" ]]; then
  LAUNCH_EXTRA_ARGS+=("translationLookaheadTime:='${TRANSLATION_LOOKAHEAD}'")
fi
if [[ -n "${ROTATION_LOOKAHEAD}" ]]; then
  LAUNCH_EXTRA_ARGS+=("rotationLookaheadTime:='${ROTATION_LOOKAHEAD}'")
fi
if [[ -n "${MOVING_GOAL_REFERENCE_FILE}" ]]; then
  LAUNCH_EXTRA_ARGS+=("movingGoalReferenceFile:='${MOVING_GOAL_REFERENCE_FILE}'")
fi
if [[ -n "${AUTO_LOAD_MOVING_GOAL_REFERENCE}" ]]; then
  LAUNCH_EXTRA_ARGS+=("autoLoadMovingGoalReferenceFile:='${AUTO_LOAD_MOVING_GOAL_REFERENCE}'")
fi
if [[ -n "${USE_OBSERVED_JOINT_TARGET_WHILE_MOVING}" ]]; then
  LAUNCH_EXTRA_ARGS+=("useObservedJointTargetWhileMoving:='${USE_OBSERVED_JOINT_TARGET_WHILE_MOVING}'")
fi
...
cleanup() {
  ...
  printf 'root\n' | sudo -S -k docker exec -i "${CONTAINER_NAME}" bash -lc \
    "pkill -f 'legged_robot_sqp_mpc|legged_robot_ipm_mpc|hexbot_runtime_mrt|hexbot_cmd_vel_target|hexbot_auto_gait_command|robot_state_publisher|ros2 launch ocs2_hexbot_legged_robot_ros hexbot_sqp.launch.py' || true" \
    >/dev/null 2>&1 || true
  python3 "${ISAAC_RUNTIME_CONTROL}" stop >/dev/null 2>&1 || true
}

ensure_container_running() {
  printf 'root\n' | sudo -S -k env DOCKER_BIN=docker bash "${CONTAINER_RUN_SCRIPT}" >/dev/null
}
...
then
  ...
  python3 "${WORKSPACE_ROOT}/src/hexbot_locomotion_mpc/src/probe_hexbot_closed_loop.py" \
    ...
    --reference-file "${REFERENCE_FILE}" \
    --no-require-backend \
    --output "${PROBE_JSON}" >/dev/null || true
...
python3 - "${CONTAINER_LOG}" "${SUMMARY_JSON}" "${REFERENCE_FILE}" "${DURATION_SEC}" "${PROBE_JSON}" "${POSTURE_JSON}" "${LINEAR_X}" "${ANGULAR_Z}" "${PROBE_DURATION_SEC}" "${TOPIC_CAPTURE_JSON}" "${RUNTIME_TOPICS_JSON}" "${PRECOMP_JSON}" "${SOLVER_TRACE_JSON}" "${SOLVER}" "${TRANSLATION_LOOKAHEAD}" "${ROTATION_LOOKAHEAD}" "${MOVING_GOAL_REFERENCE_FILE}" "${AUTO_LOAD_MOVING_GOAL_REFERENCE}" "${USE_OBSERVED_JOINT_TARGET_WHILE_MOVING}" <<'PY'
import json
import pathlib
import sys
...
solver = sys.argv[14]
translation_lookahead = sys.argv[15]
rotation_lookahead = sys.argv[16]
moving_goal_reference_file = sys.argv[17]
auto_load_moving_goal_reference = sys.argv[18]
use_observed_joint_target_while_moving = sys.argv[19]
...
summary = {
    ...
    "target_generator": {
        "translation_lookahead_time_sec": (
            float(translation_lookahead) if translation_lookahead else None
        ),
        "rotation_lookahead_time_sec": (
            float(rotation_lookahead) if rotation_lookahead else None
        ),
        "moving_goal_reference_file": moving_goal_reference_file or None,
        "auto_load_moving_goal_reference": (
            auto_load_moving_goal_reference.lower() == "true"
            if auto_load_moving_goal_reference
            else None
        ),
        "use_observed_joint_target_while_moving": (
            use_observed_joint_target_while_moving.lower() == "true"
            if use_observed_joint_target_while_moving
            else None
        ),
    },
    "counts": {
        "initial_policy_received": count("Initial MPC policy received. Runtime MRT loop is active."),
        "bootstrap_reset_request_sent": count("Bootstrap reset request sent to MPC node. Waiting for acknowledgement."),
        "fault_reset_request_sent": count("Fault reset request sent to MPC node. Waiting for acknowledgement."),
        "reset_request_sent": count("Fault reset request sent to MPC node. Waiting for acknowledgement."),
        "reset_request_sent_total": (
            count("Bootstrap reset request sent to MPC node. Waiting for acknowledgement.")
            + count("Fault reset request sent to MPC node. Waiting for acknowledgement.")
        ),
