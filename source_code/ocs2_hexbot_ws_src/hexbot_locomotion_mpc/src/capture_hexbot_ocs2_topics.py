...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--solver-timeout-threshold-ms", type=float, default=10.0)
...
class Ocs2TopicCapture(Node):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.solver_last_ms_samples: list[float] = []
    ...
    def _on_solver(self, message: DiagnosticArray) -> None:
        ...
        for status in message.status:
            for pair in status.values:
                if str(pair.key) != "solve_time_last_ms":
                    continue
                solve_time_last_ms = safe_float(pair.value)
                if solve_time_last_ms is not None:
                    self.solver_last_ms_samples.append(solve_time_last_ms)
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        timeout_threshold_ms = float(self.args.solver_timeout_threshold_ms)
        timeout_exceed_count = sum(
            1 for value in self.solver_last_ms_samples if value > timeout_threshold_ms
        )
        solver_timing_summary = {
            "available": len(self.solver_last_ms_samples) > 0,
            "timeout_threshold_ms": timeout_threshold_ms,
            "solve_time_last_ms_sample_count": len(self.solver_last_ms_samples),
            "solve_time_last_ms_mean": mean(self.solver_last_ms_samples),
            "solve_time_last_ms_variance": variance(self.solver_last_ms_samples),
            "solve_time_last_ms_max": max(self.solver_last_ms_samples, default=None),
            "timeout_exceed_count": timeout_exceed_count,
            "timeout_exceed_rate": (
                float(timeout_exceed_count) / float(len(self.solver_last_ms_samples))
                if self.solver_last_ms_samples
                else None
            ),
        }
        return {
            ...
            "solver_timing_summary": solver_timing_summary,
        }
