...
GUARD_TRACE_PREFIX = "HEXBOT_BRANCH_GUARD_TRACE "
...
def load_json_records(text: str, prefix: str) -> list[dict[str, Any]]:
...
def load_traces(text: str) -> list[dict[str, Any]]:
    return load_json_records(text, TRACE_PREFIX)


def load_branch_guard_events(text: str) -> list[dict[str, Any]]:
    return load_json_records(text, GUARD_TRACE_PREFIX)
...
def has_opposite_sign(left: Any, right: Any) -> bool:
    return (
...
def summarize_guard_events(
    events: list[dict[str, Any]], top_samples: int
) -> dict[str, Any]:
    mode_counts = Counter()
    guard_joint_counts = Counter()
    opposite_sign_guard_joint_counts = Counter()
    opposite_sign_tarsus_counts = Counter()
    command_deviations: list[float] = []
    target_alignments: list[float] = []
    worst_guard_event_samples: list[dict[str, Any]] = []

    for event in events:
        planned_mode = event.get("planned_mode")
        if planned_mode is not None:
            mode_counts[str(planned_mode)] += 1

        guard_joint = event.get("guard_joint")
        if guard_joint:
            guard_joint_counts[str(guard_joint)] += 1

        if has_opposite_sign(event.get("guard_optimized"), event.get("guard_target")):
            if guard_joint:
                opposite_sign_guard_joint_counts[str(guard_joint)] += 1

        command_deviation = event.get("command_deviation_rad")
        if is_finite_number(command_deviation):
            command_deviations.append(float(command_deviation))

        target_alignment = event.get("target_alignment_rad")
        if is_finite_number(target_alignment):
            target_alignments.append(float(target_alignment))

        tarsus_snapshot = event.get("tarsus_snapshot")
        opposite_sign_tarsus = []
        if isinstance(tarsus_snapshot, dict):
            for joint_name, snapshot in tarsus_snapshot.items():
                if not isinstance(snapshot, dict):
                    continue
                optimized = snapshot.get("optimized")
                target = snapshot.get("target")
                if has_opposite_sign(optimized, target):
                    joint_name_str = str(joint_name)
                    opposite_sign_tarsus_counts[joint_name_str] += 1
                    opposite_sign_tarsus.append(joint_name_str)

        worst_guard_event_samples.append(
            {
                "time": event.get("time"),
                "planned_mode": planned_mode,
                "guard_joint": guard_joint,
                "guard_optimized": event.get("guard_optimized"),
                "guard_target": event.get("guard_target"),
                "guard_measured": event.get("guard_measured"),
                "target_alignment_rad": target_alignment,
                "command_deviation_rad": command_deviation,
                "guard_joint_opposite_sign_vs_target": has_opposite_sign(
                    event.get("guard_optimized"), event.get("guard_target")
                ),
                "opposite_sign_tarsus_joints": opposite_sign_tarsus,
                "tarsus_snapshot": tarsus_snapshot if isinstance(tarsus_snapshot, dict) else {},
            }
        )

    worst_guard_event_samples.sort(
        key=lambda item: float(item.get("command_deviation_rad") or 0.0),
        reverse=True,
    )
    ...
    return {
        "metrics": {
            ...
            "first_time": events[0].get("time") if events else None,
            "last_time": events[-1].get("time") if events else None,
            "mode_counts": dict(mode_counts),
            "guard_joint_opposite_sign_event_count": sum(
                opposite_sign_guard_joint_counts.values()
            ),
            "guard_joint_opposite_sign_event_ratio": (
                float(sum(opposite_sign_guard_joint_counts.values())) / float(len(events))
                if events
                else None
            ),
            "command_deviation_mean_rad": safe_mean(command_deviations),
            "command_deviation_max_rad": (
                max(command_deviations) if command_deviations else None
            ),
            "target_alignment_mean_rad": safe_mean(target_alignments),
            "target_alignment_max_rad": (
                max(target_alignments) if target_alignments else None
            ),
            "tarsus_snapshot_opposite_sign_count": sum(
                opposite_sign_tarsus_counts.values()
            ),
        },
        "dominant_guard_joints": dict(guard_joint_counts.most_common(8)),
        "opposite_sign_guard_joints": dict(
            opposite_sign_guard_joint_counts.most_common(8)
        ),
        "opposite_sign_tarsus_joints": dict(
            opposite_sign_tarsus_counts.most_common(8)
        ),
        "worst_guard_event_samples": worst_guard_event_samples[
...
def main() -> int:
    ...
    branch_guard_events = load_branch_guard_events(text)
    payload = {
        "log_file": str(args.log.resolve()),
        "thresholds": {
            "base_sink_threshold_m": float(args.base_sink_threshold_m),
        },
        "branch_guard": summarize_guard_events(
            branch_guard_events,
